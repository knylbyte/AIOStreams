...
<Callout type="info" title="Segment Buffering vs. Segment Spooling">
  **Segment Buffering** keeps prefetched, decoded segments in memory and uses
  the existing compatible data path. **Segment Spooling** decodes segment data
  incrementally and places prefetched data in a transient disk spool before it
  is sent to the player. Both modes can be combined with the existing
...
| `USENET_STREAMING_MODE`                       | Streaming Mode                       | enum     | `segment_buffering` | How decoded Usenet segments are transported to the player. **segment_buffering** uses the existing compatible, RAM-oriented data path. **segment_spooling** uses bounded memory and a transient disk spool. This setting is independent of the performance profile. _(restart required)_                                                                                                                                                                                |
...
| `USENET_SEGMENT_MEMORY_CACHE_BYTES`           | Segment Memory Cache Bytes           | size     | `0`                 | The memory budget for the decoded-segment arena. **0** (the default) sizes it automatically for the selected streaming mode; it does not disable the arena. _(restart required)_                                                                                                                                                                                                                                                                                        |
| `USENET_SEGMENT_SPOOLING_MEMORY_BUDGET_BYTES` | Segment Spooling Memory Budget Bytes | size     | `128000000`         | The global hard memory budget for transient buffers and queues owned by segment spooling. It is used only in **segment_spooling** mode. _(restart required)_                                                                                                                                                                                                                                                                                                            |
| `USENET_SEGMENT_SPOOLING_STREAM_BUFFER_BYTES` | Segment Spooling Stream Buffer Bytes | size     | `8000000`           | The maximum buffer share reserved for each active HTTP stream in **segment_spooling** mode. _(restart required)_                                                                                                                                                                                                                                                                                                                                                        |
| `USENET_SEGMENT_SPOOLING_SPOOL_BYTES`         | Segment Spooling Spool Bytes         | size     | `2000000000`        | The global hard disk budget for transient segment spool files. This is separate from the persistent segment disk cache. _(restart required)_                                                                                                                                                                                                                                                                                                                            |
| `USENET_SEGMENT_SPOOLING_MIN_FREE_DISK_BYTES` | Segment Spooling Min Free Disk Bytes | size     | `512000000`         | The free-space safety margin retained on the spool filesystem in **segment_spooling** mode. _(restart required)_                                                                                                                                                                                                                                                                                                                                                        |
